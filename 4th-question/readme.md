# จงออกแบบฐานข้อมูล ระบบการเรียนการสอน โดยมีขอบเขตดังนี้

1. นักเรียนหนึ่งคนสามารถได้เรียนได้หลายวิชา
2. หนึ่งรายวิชา นักเรียนสามารถลงทะเบียนเรียนได้หลายคน 
3. ครูหนึ่งคนสอนได้หนึ่งรายวิชา และหนึ่งรายวิชามีครูสอนได้หนึ่งคน
4. เก็บข้อมูลรายละเอียดของการสอนของครู และการเข้าเรียนของนักเรียน ในรายวิชานั้นๆโดยให้แจกแจงรายละเอียดดังต่อ ไปนี้

    a. ชื่อ ตาราง

    b. ชื่อ column

    c. Primary key/Foreign key (ถ้ามี)

    d. Index ที่เป็นไปไดว้า่ น่าจะมีประโยชน์(ถ้ามี)


## answer

[demo on sqlfiddle](https://sqlfiddle.com/postgresql/online-compiler?id=9cd7b349-d7ab-4bfc-8246-323a2f4b25b4)

![example](./exam-db.png)

tool: dbeaver (erd diagram), postgresql

ai: supabase (ให้ ai gen ให้แล้วมาแก้ไขบางส่วน เพราะก่อนหน้านี้ไม่รู้จะตั้งชื่อฟิลด์อะไร)
...
```
 ...
 table subjects.code
 -- ผู้สอนมีแค่คนเดียว
 table subjects.teacher_id
 -- นักเรียนไม่สามารถลงทะเบียนวิชาเดิมซ้ำได้
 table enrollment(student_id, subject_id)
 -- เช็คเข้าเรียนชื่อได้ครั้งเดียว ต่อคลาส
 table attendance(enrollment_id, class_session_id)
```

Index ที่มี
```
idx_subjects_teacher_id ON public.subjects(teacher_id);
idx_subjects_code ON public.subjects(code);

idx_enrollments_student_id ON public.enrollments(student_id);
idx_enrollments_subject_id ON public.enrollments(subject_id);

idx_enrollments_student_subject ON public.enrollments(student_id, subject_id);

idx_class_sessions_subject_id ON public.class_sessions(subject_id);
idx_class_sessions_teacher_id ON public.class_sessions(teacher_id);
idx_class_sessions_date ON public.class_sessions(session_date);

idx_attendances_enrollment_id ON public.attendances(enrollment_id);
idx_attendances_session_id ON public.attendances(class_session_id);
idx_attendances_session_status ON public.attendances(class_session_id, status);
```
